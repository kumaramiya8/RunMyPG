...
export default function ReceiptPage() {
  ...
  const { data: orgSettings, loading: orgLoading } = useQuery(
    () => getReceiptSettings(orgId!),
    [orgId]
  )

  if (paymentLoading || orgLoading) return <ListSkeleton rows={6} />
  ...
  if (paymentError || !payment) {
    return (
      <div className="px-4 py-8 text-center">
        ...
        <Link href="/bills" className="text-xs text-primary font-semibold mt-3 inline-block">
          Back to Bills
        </Link>
      </div>
    )
  ...
  if (!tenant || !room) {
    return (
      <div className="px-4 py-8 text-center">
        <p className="text-sm text-slate-500">Could not load tenant or room details.</p>
        <Link href="/bills" className="text-xs text-primary font-semibold mt-3 inline-block">
          Back to Bills
        </Link>
      </div>
    ...
    <div className="px-4 py-4 md:px-8 md:py-6 max-w-lg mx-auto">
      <div className="no-print flex items-center gap-3 mb-5">
        ...
        <div>
          ...
          <p className="text-xs text-slate-500">Receipt for recorded payment</p>
        </div>
      </div>
